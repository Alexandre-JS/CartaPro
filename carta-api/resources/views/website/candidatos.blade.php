...
<section class="pv-section pv-plans-section" id="planos"><div class="container"><div class="pv-section-heading text-center mx-auto"><span class="pv-kicker">Escolha o seu caminho</span><h2>Comece gratuitamente. Evolua quando quiser.</h2><p>O essencial está disponível desde o primeiro dia. O plano Plus acrescenta ferramentas para uma preparação mais completa.</p></div>
    <div class="pv-plans-grid">
        @foreach($plans as $plan)
            @php($isPlus = $plan->code === 'plus')
            @php($isSchool = $plan->code === 'school')
            @php($featureLabels = ['conteudo_essencial'=>'Conteúdo essencial','sinais'=>'Biblioteca de sinais','treino_limitado'=>'Treino inicial','simulados_limitados'=>'Simulados essenciais','progresso_basico'=>'Progresso básico','simulados_ilimitados'=>'Simulados ilimitados','revisao_inteligente'=>'Revisão inteligente','prontidao'=>'Indicador de prontidão','treino_personalizado'=>'Treino personalizado','historico_completo'=>'Histórico completo','recursos_premium'=>'Recursos premium','painel'=>'Painel da escola','turmas'=>'Gestão de turmas','instrutores'=>'Instrutores','testes'=>'Testes e tarefas','resultados'=>'Resultados e analytics','analytics'=>'Analytics por turma','banco_privado'=>'Banco privado'])
            <article class="pv-plan-card {{ $isPlus ? 'is-featured' : '' }}">
                @if($isPlus)<span class="pv-plan-badge">Recomendado</span>@endif
                <div class="pv-plan-card-head"><span class="pv-plan-audience"><i class="bi bi-{{ $isSchool ? 'building' : ($isPlus ? 'stars' : 'person') }}" aria-hidden="true"></i>{{ $isSchool ? 'Para escolas' : 'Para candidatos' }}</span><h3>{{ $plan->name }}</h3><p>{{ $plan->description }}</p></div>
                <div class="pv-plan-price">@if($isSchool)<strong>Por aluno ativo</strong><span>modelo escolar</span>@elseif((float) $plan->price > 0)<strong>{{ number_format((float) $plan->price, 0, ',', '.') }}</strong><span>{{ $plan->currency }}@if($plan->duration_days) / {{ $plan->duration_days }} dias @endif</span>@else<strong>Grátis</strong><span>para começar</span>@endif</div>
                <ul>@foreach($plan->features ?: [] as $feature)<li><i class="bi bi-check2" aria-hidden="true"></i>{{ $featureLabels[$feature] ?? str($feature)->replace('_',' ')->ucfirst() }}</li>@endforeach</ul>
                <div class="pv-plan-action">@if($isSchool)<a class="pv-btn pv-btn-secondary" href="{{ route('website.schools') }}">Falar com a equipa <i class="bi bi-arrow-right" aria-hidden="true"></i></a>@elseif(config('prontovia.web_app_url') || config('prontovia.android_url'))<a class="pv-btn {{ $isPlus ? 'pv-btn-accent' : 'pv-btn-secondary' }}" href="{{ config('prontovia.web_app_url') ?: config('prontovia.android_url') }}">{{ $isPlus ? 'Escolher Plus' : 'Começar grátis' }} <i class="bi bi-arrow-right" aria-hidden="true"></i></a>@else<span class="pv-plan-coming-soon">Disponível em breve</span>@endif</div>
            </article>
        @endforeach
    </div>
    <p class="pv-plans-note"><i class="bi bi-shield-check" aria-hidden="true"></i> Sem compromisso. Pode começar no plano Free e decidir mais tarde.</p>
</div></section>
