...
@section('content')
<form class="card form-card" enctype="multipart/form-data" method="POST" action="{{ $sign->exists ? route('admin.signs.update',$sign) : route('admin.signs.store') }}">@csrf @if($sign->exists)@method('PUT')@endif
<div class="form-grid">
    ...
    <div class="field"><label>Ficheiro SVG</label><input type="file" name="svg" accept=".svg,image/svg+xml" @required(!$sign->exists)>
        @if($sign->exists && $sign->file_path)<small>Atual: {{ $sign->file_path }}</small>@endif
    </div>
