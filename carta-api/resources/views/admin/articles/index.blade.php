...
<div class="legal-library-page">
    ...
    <form method="get" class="data-toolbar legal-library-filters" aria-label="Pesquisar artigos legais">
        ...
        <label class="field"><span>Capítulo</span><select name="chapter">
                <option value="">Todos os capítulos</option>@foreach($capitulos as $capitulo)<option value="{{ $capitulo->chapter_number }}" @selected(request('chapter')==$capitulo->chapter_number)>Cap. {{ $capitulo->chapter_number }} — {{ $capitulo->chapter_title ?: 'Sem título' }} ({{ $capitulo->total }})</option>@endforeach
            </select></label>
    ...
    <x-admin.table class="legal-library-table" labelledby="legal-library-title" caption="Artigos do Código">
        <x-slot:head>
            <tr>
                ...
                <th scope="col">Título e conteúdo</th>
                <th scope="col">Acesso</th>
                <th scope="col" class="pv-actions-column">Ações</th>
            </tr>
        </x-slot:head>
        @forelse($articles as $article)
        <tr>
            <td class="legal-number"><strong>{{ $article->number }}</strong><small>Artigo</small></td>
            <td>@if($article->chapter_number)<x-admin.state type="active">Cap. {{ $article->chapter_number }}</x-admin.state><small class="legal-meta">{{ $article->chapter_title ?: 'Sem título' }}</small>@else<x-admin.state type="review">Sem capítulo</x-admin.state>@endif</td>
            <td class="legal-content"><strong>{{ $article->title }}</strong><small>{{ str($article->text)->limit(145) }}</small></td>
            <td><x-admin.state :type="$article->is_active ? 'approved' : 'neutral'">{{ $article->is_active ? 'Ativo' : 'Inativo' }}</x-admin.state><small class="legal-plan">{{ $article->is_locked ? 'Plano completo' : 'Gratuito' }}</small></td>
            <td class="actions"><x-admin.row-actions :view-href="route('admin.articles.show', $article)" label="Ações do artigo"><a href="{{ route('admin.articles.edit', $article) }}" role="menuitem"><i class="bi bi-pencil" aria-hidden="true"></i>Editar</a>@if(auth()->user()->isAdmin())<form method="POST" action="{{ route('admin.articles.destroy', $article) }}" onsubmit="return confirm('Remover este artigo?')" role="menuitem">@csrf @method('DELETE')<button type="submit" class="is-danger"><i class="bi bi-trash3" aria-hidden="true"></i>Remover</button></form>@endif</x-admin.row-actions></td>
        </tr>
        @empty
